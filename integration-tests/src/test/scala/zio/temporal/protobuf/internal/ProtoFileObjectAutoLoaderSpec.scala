...
import com.example.testing.{TestingProto2Proto, TestingProtoProto, ProtobufTestsProto}
...
class ProtoFileObjectAutoLoaderSpec extends AnyWordSpec with Matchers {
  "ProtoFileObjectAutoLoader" should {
    "load all user generated proto files via reflection" in {
      ...
      val expectedResult = List(TestingProtoProto, TestingProto2Proto, ProtobufTestsProto)
