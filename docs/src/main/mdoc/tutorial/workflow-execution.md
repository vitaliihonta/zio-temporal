...
1. Aquire a stub instance of the Workflow trait, and
...
Recall that a Workflow Definition is a trait and an implementing class.  The trait declares, and the class defines, the method that will start the Workflow Execution.  This trait and method are annotated for Temporal to distinguish them from non-workflow traits and methods.  In our running example, the Workflow type is `HelloWorld`:
...
Again, we can understand this ZIO effect type by inspecting its three type parameters.  The first type parameter, the required dependencies, is `Any`.  If any type can satisfy the requirement then effectively there is no requirement, and therefore this tells us that there are no dependencies required to run this effect.

The error type of `Nothing` we have seen before; it tells us that this effect cannot return an error.  In others words, if the effect completes, then it must complete successfully by returning a value of the third type: `ZWorkflowStub & HelloWorld`.
...
## Acquire and Execute a Workflow Stub in a Single Effect
...
There is a difference in this `for` expression from the one we saw earlier.  On each generator line in a `for` expression, the expression on the right of the `<-` is some kind of “container” of a value we are interested in, and the name on the left of the `<-` represents the value “unwrapped” from the container.  The meaning of “container” and “unwrap” depends on the specific types.  In the case of a ZIO effect, unwrapping means to run the effect, and the returned value is what is contained.

In the `for` expression in the previous section we ignored the contained values because we had no need to use them.  That composed `for` expression itself returned a ZIO effect containing the `Unit` value, as determined by the last line `yield()`.

In this case, we do care about the contained values: we care about the value that is the Workflow stub contained in the first effect, because we need it to execute the Workflow.  And we care about the value returned by the Workflow Execution, which is contained in the second effect, because that is the value to be returned to the user who runs the application.  The value returned by the second effect is what the composed effect will return.  The Workflow stub is only needed to execute the Workflow and can be discarded after that.

In the `for` expression here, `helloWorld` to the left of `<-` on the first generator line represents the value that will be returned by the first effect when it is run, and the second generator declares that when that stub instance will be available, its `apply()` method is to be invoked by the `ZWorkflowStub.execute()` method.

The `result` to the left of the `<-` on the second generator line represents the value returned by the second effect, and is declared to be the value contained in the composed effect in the closing line `yield result`.
...
1. `client`, an instance of `ZWorkflowClient`, which has the `newWorkflowStub[]()` method, and
2. `workflowOptions`, an instance of `ZWorkflowOptions`, which `newWorkflowStub[]()` requires as a value parameter.
...
The [`ZWorkflowOptions`](https://zio-temporal.vhonta.dev/api/zio/temporal/workflow/ZWorkflowOptions$) singleton object gives us the `withWorkflowId()` method, and returns an object with the [`withTaskQueue()`](https://zio-temporal.vhonta.dev/api/zio/temporal/workflow/ZWorkflowOptions$$SetTaskQueue.html#withTaskQueue(value:String):zio.temporal.workflow.ZWorkflowOptions) method.

We used the Task Queue name already when we started the Worker.  Each Worker listens to exactly one Task Queue, so the name provided when executing a Workflow must match the name recognized by any Worker that is to perform the execution.  The name is a `String` value.
...
will give us a `String` whose value is an informative and unique indentifier for a Workflow Execution.
...
To avoid this situation, ZIO provides some services that can behave as expected when running in production, but that come with test implementations that can be used so that tests functions will be applied to the same input every time they are invoked.  One such service is [`Clock`](https://javadoc.io/doc/dev.zio/zio_3/latest/zio/Clock$.html), which has a [`currentDateTime`](https://javadoc.io/static/dev.zio/zio_3/2.1.9/zio/Clock$.html#currentTime-faf) method that returns a
...
You may find one way more readable than the other.  Which you prefer is a matter of style.  Either way, we now have the `ZWorkflowOptions` instance that the `ZWorkflowClient` needs.  Next, we will see how to acquire that `ZWorkflowClient` instance.
...
`ZIO.serviceWithZIO[]()` takes a type parameter, and a value parameter.  The type parameter is a dependency to be injected, in this case `ZWorkflowClient`.  The value parameter is a function whose input parameter is the dependency and whose return value is the effect that will use that dependency, in this case the `for` expression that starts the Workflow Execution.

To be specific: we have a `for` expression that contains an undefined variable `client`:
...
We define a function that takes an argument of the type of the undefined variable returns the `for` expression:
...
and finally we pass this function as the single argument to the `serviceWithZIO[]` method:
...
The type returned by invoking this `serviceWithZIO[ZWorkflowClient]` is:
...
The input value to this workflow (`"Alice"`) is hard-coded in the `for` expression.  Let’s instead use a command-line argument to provide that value for the Workflow Execution.

## Depend On a Value That Is Not a ZIO Service

We just saw an example of how the ZIO dependency injection features give our program access to a necessary ZIO service, in this case a `ZWorkflowClient`.  Sometimes our program needs a value that is not provided by a ZIO library, in this case a user-provided `String` that will be the input value to the Workflow Execution.  We can do this even more easily than when using `serviceWithZIO`.

Again, the type of the `serviceWithZIO[]` expression above is:
...
where the returned value is the `serviceWithZIO[]` expression using the value of the input `String` as the argument to `execute(HelloWorld())`.  Let’s call this function `invokeWorkflow`:
...
We can apply this function `invokeWorkflow` to any `String` value to get a runnable ZIO effect that passes that `String` as the input value to the Workflow Execution.  The [`ZIOAppArgs`](https://javadoc.io/doc/dev.zio/zio_3/latest/zio/ZIOAppArgs$.html#) object gives us a way to get a `String` from the command-line that ran the program, the [`getArgs`](https://javadoc.io/static/dev.zio/zio_3/2.1.9/zio/ZIOAppArgs$.html#getArgs-63f) method.

This `getArgs` method returns a ZIO effect, and therefore we could have included it in the `for` expression we constructed above.  However, to keep the user input/output separate from the Workflow execution, we will make a different `for` expression to create a ZIO effect to (1) get the command-line argument, (2) run the Workflow execution effect, and (3) displays the results to the user.

## Write a Program the User Can Run

With the ZIO effect returned by the `for` expression above, we can give it a `String` input and it will return a `String` output.  We want to get our input `String` from the command line, which we can do using [`getArgs`](https://javadoc.io/static/dev.zio/zio_3/2.1.9/zio/ZIOAppArgs$.html#getArgs-63f).  `getArgs`, when invoked, returns a ZIO effect that returns a `Seq[String]`, which is an ordered collection of `String` values.  If `chunk` is a value of type `Seq[String]` then `chunk(0)` returns the first element.  If a `Seq[String]` contains the values of command-line arguments, then the first element is the first command-line argument.

There are a couple ways to get at the first element of the `Seq[String]` when that `Seq[String]` is "contained" inside a ZIO effect (_i.e., is its return value when run).  In an earlier section, we saw that we can include variable definitions in a `for` expression, like this:
...
These are ways to get the input value from the command line.  Printing the output value to the user’s console is so simple is hardly merits discussion: the `Console.printLine()` method.  Knowing how to read the input and write the output, we can now write a three-line program that: (1) reads a name from the command line, (2) executes the Workflow with that name as input to the `invokeWorkflow` function we defined earlier, and (3) prints the result for the user:
...
Alternatively, if you run from the `sbt` prompt then omit the quotes

```bash
run Bob
```

Because both the Worker-starting program and the Workflow-starting program are in the same SBT project, SBT will prompt you to choose one.
