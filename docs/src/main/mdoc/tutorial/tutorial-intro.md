...
The type of this `ZIO` has three type parameters, referred to as `R`, `E`, and `A`, in that order.  `R` are the required dependencies for the program to run, `E` is the possible error type if the program fails when running, and `A` is the type returned by the program when the run completes successfully.  The object returned by evaluating `printLine("Hello world!")` has the type:
