...
/** Helper instance representing workflow state.
  * Can be either uninitialized or containing value
  *
  * @tparam A the state value
  */
class ZWorkflowState[A] private[ztemporal] (private var underlying: Option[A]) {
  ...
  /** Returns true if this State is initialized '''and''' the predicate
    * $p returns true when applied to this state value.
  ...
  /** Returns true if this state is uninitialized '''or''' the predicate
    * p returns true when applied to this state value.
    *
    *  @param  p   the predicate to test
    */
  ...
  /** Takes a snapshot of the state applying a function to it ors throw an error
    ...
    * @return the result of function application on this state
    */
  @throws[NoSuchElementException]
  ...
  /** Takes a snapshot of the state or returns the provided default value
    ...
    * @return the state or default value
    */
  def snapshotOrElse(default: => A): A = underlying.getOrElse(default)

  /** Converts this state to [[Option]]
    * @return the state value or [[None]]
    */
  def toOption: Option[A] = underlying

  /** Converts this state to [[Either]]
    * @tparam E error type
    * @param left error value which will be used when state is not initialized
    * @return the state value or [[Left]]
    */
