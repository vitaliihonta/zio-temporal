...
import ztemporal.ZCurrentTimeMillis
import ztemporal.ZSearchAttribute
import ztemporal.ZWorkflowExecution
import ztemporal.ZWorkflowInfo
import ztemporal.activity.ZActivityStubBuilderInitial
import ztemporal.activity.ZLocalActivityStubBuilderInitial
...
object ZWorkflow {
  ...
  /** Retrieves workflow info
    *
    * @see [[io.temporal.workflow.WorkflowInfo]]
    * @return [[ZWorkflowInfo]]
    */
  ...
  /** Suspends workflow execution until the given duration elapsed
    ...
    * @see [[Workflow.sleep]]
    * @return unblocks when duration elapsed
    */
  ...
  /** Suspends workflow execution while the given predicate holds
    *
    * @param cond await condition
    * @see [[Workflow.await]]
    * @return unblocks when condition becomes false
    */
  ...
  /** Suspends workflow execution while either the given predicate holds or timeout elapsed
    ...
    * @see [[Workflow.await]]
    * @return unblocks when condition becomes false or timeout elapsed
    */
  ...
  /** Suspends workflow execution until the given predicate holds
    ...
    * @param cond await condition
    * @see [[Workflow.await]]
    * @return unblocks when condition becomes true
    */
  ...
  /** Suspends workflow execution until either the given predicate holds or timeout elapsed
    ...
    * @param timeout await timeout
    * @see [[Workflow.await]]
    * @return unblocks when condition becomes true or timeout elapsed
    */
  ...
  /** Wraps a procedure in a CancellationScope. The procedure receives the wrapping CancellationScope
    ...
    * @see [[Workflow.newCancellationScope]]
    * @param thunk code to wrap in the cancellation scope
    * @return wrapped proc
    */
  ...
  /** Creates a CancellationScope that is not linked to a parent scope.
    * [[ZCancellationScope.run()]] must be called to execute the code the scope wraps.
    *  The detached scope is needed to execute cleanup code after a workflow is canceled which cancels the root scope
    * that wraps the @WorkflowMethod invocation. Here is an example usage:
    *
    * @see [[Workflow.newDetachedCancellationScope]]
    * @param thunk parameter to wrap in a cancellation scope.
  ...
  /** Returns current timestamp
    ...
    * @see [[Workflow.currentTimeMillis()]]
    * @return current time millis as [[ZCurrentTimeMillis]]
    */
  ...
  /** Adds or updates workflow search attributes.
    *
    * @see [[Workflow.upsertSearchAttributes]]
    * @param attrs map of String to [[ZSearchAttribute]] value that can be used to search in list APIs
    */
  ...
  /** Creates a builder of client stub to activities that implement given interface.
    *
    * @tparam A activity interface
    * @return activity stub builder
    */
  ...
  /** Creates a builder of client stub to local activities that implement given interface.
    *
    * @tparam A activity interface
    * @return local activity stub builder
    */
  ...
  /** Creates a builder of client stub that can be used to start a child workflow that implements given interface.
    ...
    * @return child workflow stub builder
    */
  ...
  /** Creates client stub that can be used to communicate to an existing workflow execution.
    *
    * @tparam A workflow interface
    * @param workflowExecution execution of the workflow to communicate with.
    * @return external workflow stub
    */
