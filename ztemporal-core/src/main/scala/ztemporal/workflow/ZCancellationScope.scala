...
/**  Handle to a cancellation scope created through [[ZWorkflow.newCancellationScope]] or
  * [[ZWorkflow.newDetachedCancellationScope]].
  * Supports explicit cancelling of the code a cancellation scope wraps.
  * The code in the CancellationScope has to be executed using [[run]] method.
  */
class ZCancellationScope private[ztemporal] (val toJava: CancellationScope) extends AnyVal {
  ...
  /** Cancels the scope as well as all its children.
    *
    * @param reason human readable reason for the cancellation. Becomes message of the  CanceledException thrown.
    */
  ...
  /** When set to false parent thread cancellation causes this one to get canceled automatically.
    * When set to true only call to [[cancel()]] leads to this scope cancellation.
    */
  def isDetached: Boolean = toJava.isDetached

  /** Use this promise to perform cancellation of async operations.
    *
    * @return promise that becomes ready when scope is canceled. It contains reason value or null if none was provided.
    */
